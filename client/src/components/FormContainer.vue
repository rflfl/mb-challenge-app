	...
	<transition name="slide-fade">
		<div v-if="etapa === 1" id="etapa-1" class="form-container">
			...
			<div class="secao-tipo-pessoa">
				<div>
					<input type="radio" id="pessoa-fisica" value="fisica" name="tipo-pessoa" v-model="tipoPessoa" />
					<label for="pessoa-fisica">Pessoa física</label>
				</div>
				<div>
					<input type="radio" id="pessoa-juridica" value="juridica" name="tipo-pessoa" v-model="tipoPessoa" />
					<label for="pessoa-juridica">Pessoa jurídica</label>
				</div>
			</div>
			<div class="secao-botoes">
				<BaseButton classe="btn-default w-full" :class="!cadastro.email.trim() ||
			cadastro.email.length < 4 ||
			cadastro.email.indexOf('@') === -1
			? 'disabled'
			: ''
			" :disabled="!cadastro.email.trim() ||
			cadastro.email.length < 4 ||
			cadastro.email.indexOf('@') === -1
			? true
			: false
...
function cadastrar() {
	fetch('http://localhost:3000/registration', {
		...
		.catch(error => {
			...
			alert(`Houve um erro ao realizar o cadastro.(${error})`)
		});
}
</script>
...
@media screen and (min-width: 764px) {
	.form-container {
		width: 360px;
		margin: 0 auto;
		padding-top: 100px 15px 50px 15px;
	}
